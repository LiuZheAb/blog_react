            ...
            <div className="page-content">
                <div className="article">
                    ...
                    <div className="article-content">
                        ...
                        <p className="indent-2">这是由于前段时间想要系统地学习一下 GoJS，整理一份文档。其实最开始是在语雀上写文档的。不得不说，功能很强大，但是只能放一些普通的文字、图片、链接之类的，写了大概有四五节，突然想把其实现的效果展示在文档中，可是文档里没办法插入可交互的 HTML 元素<Emoji type="emoji-jiong" />，于是乎，就开始调研哪些网站可以满足自己的需求。</p>
                        ...
                        <p className="indent-2">想建站但是不想花钱买服务器，穷呀<Emoji type="emoji-jiong" />，怎么办？乖乖百度找办法，然后发现了 github-pages（以下简称 gp）这个好东西，可以拿来做个人博客，虽说 github 在国内访问的速度不太快吧，但是它免费呀<Emoji type="emoji-caimi" />，还挑啥。So，开始研究 gp 怎么用。</p>
                        <p className="indent-2">第一时间想到的就是最笨的方法，纯 HTML、CSS 肯定是行得通的。而且语雀有自带的将文档转为 HTML 的功能，还是很好用的，所以就想着先在语雀里写好文档，再转成 HTML，然后在需要的位置插入代码。随后把想法测试了一下，放到 gp 上没问题，Good。</p>
                        <p className="indent-2">既然都建站了，索性就弄个博客吧，上面的办法可以是可以，可是每个页面都要写个 HTML 文件，这未免也太蠢了<Emoji type="emoji-xianqi" />，想想果断放弃了。</p>
                        <p className="indent-2">然后就自然而然地开始找博客模板，然后就找到了个可以快速构建个人博客的好东西，Hexo，想了解的小伙伴请移步 <a href="https://hexo.io/" target="_blank" rel="noopener noreferrer">Hexo 官网</a>。</p>
                        ...
                        <p className="indent-2">Hexo 除了快速建站之外，还有很多主题选择，感兴趣的可以看一下<a href="https://hexo.io/themes/" target="_blank" rel="noopener noreferrer">Hexo 官方 Themes</a>，将近 300 个，真的很多，而且拉取、配置都很便捷。</p>
                        <p className="indent-2">折腾了大概有三四个小时，其实大部分时间都用在挑主题上了。。。然后就正式开始搞，然后就没有然后了<Emoji type="emoji-dai" />。。。开始用我才发现，Hexo 用 Markdown 解析文章，它只能解析文章啊，还是不能满足需求随心所欲的插 HTML 元素（也可能没学会怎么用），关键是感觉写起来还挺麻烦的，所以又放弃 Hexo了。</p>
                        ...
                        <p className="indent-2">测试了一下，将 React 打包后的代码丢到 github 上，可行，于是就这么愉快地决定了<Emoji type="emoji-ku" />。</p>
                        <br />
                        <p className="indent-2">以上基本上就是建站的整个历程。</p>
                        <br />
                        <div className="text-right">
                            <em>写于 2020-4-6 凌晨</em>
                        </div>
                        <hr />
                        <h2>略有小感</h2>
                        ...
                        <p className="indent-2">为什么在博客上花了这么多时间呢？其实我并没有在写文章上花费太多时间，因为博客目前还处于初期阶段，并未成型，所以大部分时间都用在网站调整和优化了。写到这里，我去看了下我博客源代码在 GitHub 上的 <a href="https://github.com/LiuZheAb/blog_react/commits/master" target="_blank" rel="noopener noreferrer">commits</a> 记录，发现刚好 100 条了，真是巧。</p>
                        <p className="indent-2">二十多天，我的博客的代码结构是改了又改，组件化也越来越彻底。虽然没有后端，但是通过公共 data 数据文件实现了类似数据库的功能，面包屑、菜单、页脚、目录组件内的数据统一调用此数据。网站的维护、更新工作比刚开始的时候简化了许多(刚开始的时候，要想发一篇新文章，我得手动将面包屑、菜单、页脚、目录组件逐一修改才行，效率实在是低)。</p>
                        ...
                        <br />
                        <p className="indent-2">UI 部分参考了语雀</p>
                        <br />
                    </div>
                </div>
                <Directory />
