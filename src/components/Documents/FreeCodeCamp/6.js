            ...
            <div className="page-content">
                <div className="article">
                    ...
                    <div className="article-content">
                        <h2>概述</h2>
                        <p>@keyframes 规则通过在动画序列中定义关键帧（或 waypoints）的样式来控制 CSS 动画序列中的中间步骤。这比转换更能控制动画序列的中间步骤。</p>
                        <CodeMirror value={code1} options={codeMirrorOptions} />
                        <p>JavaScript 可以通过 CSS 对象模型接口 CSSKeyframesRule 来访问 @keyframes </p>
                        <p>要使用关键帧, 先创建一个带名称的 @keyframes 规则，以便后续使用 animation-name 这个属性来将一个动画同其关键帧声明匹配。每个 @keyframes 规则包含多个关键帧，也就是一段样式块语句，每个关键帧有一个百分比值作为名称，代表在动画进行中，在哪个阶段触发这个帧所包含的样式。</p>
                        ...
                        <p>如果一个 @keyframes 里的关键帧的百分比存在重复的情况，以最后一次定义的关键帧为准。 因为@keyframes 的规则不存在层叠样式(cascade)的情况，即使多个关键帧设置相同的百分值也不会全部执行。</p>
                        <h3>属性个数不定</h3>
                        <p>如果一个关键帧中没有出现其他关键帧中的属性，那么这个属性将使用插值(不能使用插值的属性除外, 这些属性会被忽略掉)。例如：</p>
                        <CodeMirror value={code2} options={codeMirrorOptions} />
                        <p>例子中，"top"属性分别出现在 0%, 30% 和 100% 的关键帧中，"left"属性分别出现在 0%, 68% 和 100% 关键帧中.</p>
                        <h3>当关键帧被重复定义</h3>
                        <p>如果某一个关键帧出现了重复的定义，且重复的关键帧中的 css 属性值不同，以最后一次定义的属性为准。例如：</p>
                        <CodeMirror value={code3} options={codeMirrorOptions} />
                        <p>上面这个例子中，50% 关键帧中设置的属性 top: 10px 是有效的，但是其他的属性会被忽略</p>
